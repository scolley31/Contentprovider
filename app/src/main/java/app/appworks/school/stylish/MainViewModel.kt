...
class MainViewModel(private val stylishRepository: StylishRepository) : ViewModel() {
    ...
    private val _getProductSuccess = MutableLiveData<SuccessDetail>()

    val getProductSuccess: LiveData<SuccessDetail>
        get() = _getProductSuccess
    ...
    fun getDetailProduct(productId: Long) {

        coroutineScope.launch {
                val result = stylishRepository.getDetailProduct(productId)
            Log.d("test", "result = $result")

                _getProductSuccess.value = when (result) {
                    is Result.Success -> {
                        _error.value = null
                        result.data
                    }
